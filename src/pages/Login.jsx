    ...
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400'>
      ...
      <div className='bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm'>
        ...
        <form onSubmit={onSubmitHandler}>
          ...
          {/* <button disabled={isLoading} className='w-full py-2.5 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-900 cursor-pointer text-white font-medium'>{state}</button> */}
          <button
            disabled={isLoading}
            className={`w-full py-2.5 rounded-full text-white font-medium
            ${isLoading
                ? 'bg-gray-500 cursor-not-allowed'
                : 'bg-gradient-to-r from-indigo-500 to-indigo-900 cursor-pointer'
            }`}
          >
